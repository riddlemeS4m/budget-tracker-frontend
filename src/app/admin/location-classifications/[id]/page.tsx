...
import {
  ...
  useDeleteLocationClassification,
} from "@/lib/hooks";

export default function LocationClassificationDetailPage({
  params,
}: {
  ...
  const deleteLocationClassification = useDeleteLocationClassification();

  function handleDelete() {
    if (confirm("Delete this location classification?")) {
      deleteLocationClassification.mutate(lcId, {
        onSuccess: () => router.push("/admin/location-classifications"),
      });
    }
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold">
          Location Classification #{data.id}
        </h1>
        <div className="flex gap-3">
          <Link
            href={`/admin/location-classifications/${data.id}/edit`}
      ...
      <div className="mt-6">
        <Link
          href="/admin/location-classifications"
          className="text-sm text-blue-600 dark:text-blue-400 hover:underline"
        >
          ← Back to Location Classifications
        </Link>
      </div>
    </div>
  );
